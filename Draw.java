...
public class Draw extends JComponent{
	...
	public Draw(){
		...
		try{
			...
			backgroundImage = ImageIO.read(getClass().getResource("castlebackground.png"));
		}
		catch(IOException e){
			e.printStackTrace();
		}
				...
				for(int ctr = 0; ctr < 5; ctr++){
					try {
						if(ctr==4){
							...
							resource = getClass().getResource("normal/attack"+ctr+".png");
						}
		...
		Thread thread1 = new Thread(new Runnable(){
			public void run(){
				...
				for(int x=0; x<monsters.length; x++){
					if(monsters[x]!=null){
						if(monsters[x].contact){
							monsters[x].life = monsters[x].life - 20;
						}
					}
				}
			}
	...
	public void windtalkerAnimation(){
		Thread thread1 = new Thread(new Runnable(){
			public void run(){
				for(int ctr = 0; ctr < 4; ctr++){
					try {
						if(ctr==3){
							...
							resource = getClass().getResource("first/windtalker"+ctr+".png");
						}
